...
など複数の役割を持つクラスやファイルが登場した。

最初は、

- なぜクラスを分ける必要があるのか
- なぜControllerに全部書かないのか
- なぜServiceが必要なのか

が分かりにくかった。

しかし整理すると、これは「MVCモデル」という設計思想に基づいていることが分かった。
...
の3つに役割分担する設計パターン（アーキテクチャスタイル）である。

Spring Bootでも、このMVCモデルを前提としてWebアプリケーションが構成されている。
...
│         コントローラー          │              │             モデル             │
│──────────────────────────────│              │──────────────────────────────│
│ SampleController1             │              │ SampleService                 │
│ ┌──────────────────────────┐ │              │ ┌──────────────────────────┐ │
│ │         処理A             │ │              │ │         処理A             │ │
│ └──────────────────────────┘ │              │ └──────────────────────────┘ │
│                              │              └──────────────▲───────────────┘
...
という責任分離が明確に見えるようになった。

また、

「Spring BootはMVCモデルを前提に設計されている」
...
など多くの技術が登場したが、その根本にはMVCモデルによる役割分担が存在していた。
